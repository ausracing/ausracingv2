...
const TeamCard = ({
  ...
  const [imgSrc, setImgSrc] = useState(
    member.photo ? urlFor(member.photo) : nameToPath(member.name),
  );
  ...
  return (
    ...
    >
      <div
        className={`w-full aspect-[4/5] select-none relative flex items-center justify-center overflow-hidden border-b ${member.isLeader ? "border-primary" : "border-white/10"}`}
      >
        <Image
          src={imgSrc}
          alt={member.name}
...
export default function TeamClient({ members }: { members: TeamMember[] }) {
  ...
  const categories = [...new Set(members.map((m) => m.category))].filter(
    Boolean,
  );
  ...
  const validFilter = sortedFilters.includes(activeFilter)
    ? activeFilter
    : sortedFilters[0] || "Executive Board";
    ...
    <div className="min-h-screen bg-background pt-7 pb-16 px-6">
      ...
      <div className="max-w-2xl mx-auto mb-2 flex items-start justify-center px-4 min-h-[40px]">
        <p className="text-[13px] sm:text-[14px] text-white/60 text-center italic transition-opacity duration-300">
          &quot;
          {defaultDescriptions[validFilter] ||
            "Pushing the absolute limits of collegiate motorsport engineering."}
          &quot;
        </p>
      </div>
