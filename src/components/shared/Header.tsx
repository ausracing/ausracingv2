...
export default function Header() {
  ...
  useEffect(() => {
    if (pathname !== "/") {
      setTimeout(() => setActiveHash(""), 0); // Pushed to the next tick to prevent cascade
      return;
    }
    ...
    <>
      <nav className="sticky top-0 w-full z-50 flex items-center justify-between px-4 md:px-8 lg:px-12 h-17 bg-background/90 backdrop-blur-md border-b border-white/10 select-none">
        ...
        <div className="order-2 lg:order-1 flex-none lg:flex-1 flex justify-center lg:justify-start">
          <Link
            href="/"
            className="hover:opacity-80 transition-opacity flex items-center translate-y-0.5"
          >
            ...
            const navItem = (
              ...
              >
                <Link
                  href={link.href}
                  onClick={(e) =>
                    handleNavClick(e, link.href, link.name, false)
                  }
                >
                  {link.name}
                </Link>
