...
export default function DownloadPdfButton({ pdfUrl, issueName }: DownloadPdfButtonProps) {
  const cleanMonth = issueName.replace(/news/i, '').trim().replace(/\s+/g, '-').toLowerCase();
  ...
  return (
    ...
    >
      ...
      <span className="text-xs sm:text-sm font-medium whitespace-nowrap truncate">Download PDF</span>
    </a>
  );
}